...
//     将结果放在请求中
    ...
    @PostMapping("/home_records")
    @ResponseBody
    public List<Present_record> getHomeTask_issued() {
        List<Present_record> present_records = new ArrayList<>();
        List<Task_issued> task_issueds = projectService.queryTask_issued();
        int qualifiedNum = 0;
        int midqualifiedNum = 0;
        int NotqualifiedNum = 0;
        for (Task_issued task_issued : task_issueds) {
            Present_record present_record = new Present_record();
            present_record.setCheck_id(task_issued.getCheck_id());
            present_record.setSecurity_level(task_issued.getSecurity_level());
            present_record.setUnit_name(task_issued.getUnit_name());
            present_record.setSoftware_name(task_issued.getSoftware_name());
            present_record.setName(task_issued.getName());
            List<Record_item> record_items = recordService.findAllItems(task_issued.getCheck_id());
            for (Record_item record_item : record_items) {
            ...
            present_record.setQualifiedNum(qualifiedNum);
            present_record.setMidqualifiedNum(midqualifiedNum);
            present_record.setNoqualifiedNum(NotqualifiedNum);
            System.out.println(present_record);
            present_records.add(present_record);
        }
        System.out.println(present_records);
        return present_records;
    }
//
//    @PostMapping("/home_records")
//    @ResponseBody
//    public List<Task_issued> getHomeTask_issued() {
//        List<Task_issued> task_issueds = projectService.queryTask_issued();
//        return task_issueds;
//    }
